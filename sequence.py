...
class TextSequenceTestState(SequenceTestState):
    ...
    def step(self, net_out: Tuple[torch.Tensor, Optional[torch.Tensor]], data: Dict[str, torch.Tensor]):
        ...
        if not self.is_index_tensor(scores):
            ...
            oracle_ok = self.compare_direct((scores, data["out_len"].clamp_(max=scores.shape[1-self.batch_dim])),
    ...
    def plot(self) -> Dict[str, Any]:
        res = super().plot()
        res["mistake_examples"] = plot.TextTable(["Input", "Reference", "Output", "Prefix match"] +\
...
class TypedTextSequenceTestState(TextSequenceTestState):
    ...
    def plot(self) -> Dict[str, Any]:
        res = super().plot()
        res["mistake_examples"] = plot.TextTable(["Input", "Reference", "Type", "Output"],
                                                                     self.bad_sequences)
